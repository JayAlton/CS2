...
public class BinarySearch {
    ...
    public static int recurBinarySearch(int[] array, int target) {
        return recurBinarySearch(array, target, 0, array.length - 1); // FIXME
    }
    ...
    private static int recurBinarySearch(int[] array, int target, int low, int high) {
    	...
    	if(array[mid] < target) {
    		return recurBinarySearch(array, target, mid + 1, high);
    	} else if(array[mid] > target) {
    		return recurBinarySearch(array, target, low, mid - 1);
    	} else {
    		return mid;
    	}
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = -5; i < 2 * SIZE + 5; i++) {
            ...
            int actual = recurBinarySearch(numbers, i);
            if (actual != expected) {
